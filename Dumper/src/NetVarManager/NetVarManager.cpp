...
namespace Dumper
{
    namespace NetVarManager
    {
        ...
        {
            // Ищем g_pClientClassHead через стандартную сигнатуру Source Engine
            // Эта сигнатура стабильна для GMod и CS:Source
            uintptr_t clientClassPtr = pProcess->FindPattern(
                "client.dll",
                "A1 ? ? ? ? 56 8B 70 10 83 C0 04 3B F1 72 F2",
                Remote::SignatureType_t::READ, // Читаем значение по адресу
                0x1, // Пропускаем байт A1 (опкод)
                0
            );

            if (!clientClassPtr) {
                std::cout << "[-] Failed to find ClientClass signature!" << std::endl;
                return false;
            }

            // clientClassPtr теперь указывает на первый ClientClass
            for (auto Class = ClientClass(clientClassPtr); Class.Get(); Class = ClientClass(Class.GetNextClass())) {
                auto table = RecvTable(Class.GetTable());
                if (!table.Get()) continue;

                // Рекурсивно сканируем таблицу
        ...
        void CNetVarManager::ScanTable(RecvTable& table, int level, int offset, const char* name)
        {
            ...
            for (auto i = 0; i < count; ++i) {
                ...
                if (!propName.empty() && isdigit(propName[0]))
                    continue;

                auto isBaseClass = (propName == "baseclass");
                if (!isBaseClass) {
                    _tables[name].push_back({ propName, prop });
                }
